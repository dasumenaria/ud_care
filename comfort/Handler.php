...
if(isset($_POST['receipt_submit']))
{
	...
	$ledger_names=$_POST['ledger_name'];
	$ledger_type_id=$_POST['ledger_type_id'];
	$amounts=$_POST['amount'];
	$invoice_lists=$_POST['invoice_list'];
	$tds_amnt=$_POST['tds_amnt'];
	$discount=$_POST['discount'];
	$remaining_amount=0;
	
	$totalamount = $tds_amnt+$discount; 

	foreach ($ledger_names as $key => $ledger_name) {
			$result_whom=mysql_query("select `id` from `ledger_master` where `ledger_type_id`='".$_POST['ledger_type_id']."' && `name`='".$ledger_name."'");
			$res_whom=mysql_fetch_array($result_whom);
			$ledger_acc_id=$res_whom['id'];

			foreach ($invoice_list[$ledger_name] as $invoice_id) {
			$result_whom=mysql_query("select * from `invoice_detail` where `invoice_id`='".$_POST['invoice_id']."'");
			$res_whom=mysql_fetch_array($result_whom);
			$amount=$res_whom['amount'];
			$amount=$amount-$amount[$key];
			$update_amount = mysql_query("update invoice_detail set amount = '$amount' 
				where invoice_id='".$_POST['invoice_id']."' ");
	        }

	    $totalamount = $totalamount + $amounts[$key];
		$amount=$amounts[$key];

		if($key==0){
	     $amount=$amount+$tds_amnt+$discount;
	     
	    }
		if($remaining_amount>0){
			$amount=$amount+abs($remaining_amount);
		}
		
		foreach ($invoice_lists[$ledger_name] as $key2 => $invoice_id) {
			...
			if(mysql_num_rows($check_receipt_detail)>0)
			{
			   $row_receipt_detail=@mysql_fetch_array($check_receipt_detail);
				$due_amount_of_invoice=$row_receipt_detail['due_amnt'];
	    	}
			else
			{
				$result=mysql_query("select `grand_total` from `invoice` where `id`='".$invoice_id."'");
				$row=mysql_fetch_array($result);
				$due_amount_of_invoice=$row['grand_total'];	
			}

			$remaining_amount=$amount-$due_amount_of_invoice;
//			echo 'r='.$remaining_amount.'<br/>';
			if($remaining_amount>=0){
				...
				if(mysql_num_rows($receipt_fetch)>0)
				{
					...
					@mysql_query("update `receipt_detail` set `due_amnt`='".abs($remaining_amount)."' where `invoice_id`='".$invoice_id."'");
					break;
				
				}
				else
				{
					@mysql_query("insert into `receipt_detail` set `invoice_id`='".$invoice_id."',`due_amnt`='".abs($remaining_amount)."' ");
					break;
				}
			}
			...
			$sql = "select `id` from `ledger_master` where `ledger_type_id`='".$_POST['ledger_type_id']."' && `name`='".$ledger_name."'";	
			$result_whom=mysql_query($sql);
			$res_whom=mysql_fetch_array($result_whom);
			@$invoice_list=implode(",",$_POST['invoice_list'][$ledger_name]);

		    if($receipt_type=="cash")
			 {
				$rs=@mysql_query("insert into `ledger` set `date`='".datefordb($_POST['date'])."',`name`='".$ledger_name."',`ledger_master_id`='".$ledger_acc_id."',`credit`='".$amounts[$key]."',`debit`='0',`invoice_id`='".$invoice_list."',`transaction_type`='receipt',`transaction_id`='".$_POST['receipt_id']."',`narration`='".$_POST['narration']."',`current_date`='".date("Y-m-d")."'");
				//$rs1=@mysql_query("insert into `ledger` set `date`='".datefordb($_POST['date'])."',`name`='Cash Account',`ledger_master_id`='".cash_account_id()."',`credit`='".$amounts[$key]."',`debit`='0',`invoice_id`='".$invoice_list."',`transaction_type`='receipt',`transaction_id`='".$_POST['receipt_id']."',`narration`='".$_POST['narration']."',`current_date`='".date("Y-m-d")."'");
				
			}
			else
			{
				$rs=@mysql_query("insert into `ledger` set `date`='".datefordb($_POST['date'])."',`name`='".$ledger_name."',`ledger_master_id`='".$ledger_acc_id."',`credit`='".$amount[$key]."',`debit`='0',`invoice_id`='".implode(',', $invoice_list[$ledger_name])."',`transaction_type`='receipt',`transaction_id`='".$_POST['receipt_id']."',`narration`='".$_POST['narration']."',`current_date`='".date("Y-m-d")."'");
					$bank_res=mysql_query("select `name` from `bank_reg` where `id`='".$_POST['bank_id']."'");
					$row_bank=mysql_fetch_array($bank_res);
	...
	$rs1=@mysql_query("insert into `ledger` set `date`='".datefordb($_POST['date'])."',`name`='Cash Account',`ledger_master_id`='".cash_account_id()."',`credit`='0',`debit`='".$totalamount."',`invoice_id`='".$invoice_list."',`transaction_type`='receipt',`transaction_id`='".$_POST['receipt_id']."',`narration`='".$_POST['narration']."',`current_date`='".date("Y-m-d")."'"); 
		 if(!empty($_POST['discount']))
			{
				$ledger_master=mysql_query("select `id` from `ledger_master` where `name`='Discount' && `ledger_type_id`='5'");
				$row_ledger_master=mysql_fetch_array($ledger_master);
				
				$rs2=mysql_query("insert into `ledger` set `date`='".datefordb($_POST['date'])."',`name`='Discount',`ledger_master_id`='".$row_ledger_master['id']."',`credit`='".$_POST['discount']."',`debit`='0',`invoice_id`='".$invoice_list."',`transaction_type`='receipt',`transaction_id`='".$_POST['receipt_id']."',`narration`='".$_POST['narration']."',`current_date`='".date("Y-m-d")."'");
			}
		...
		if(!empty($_POST['tds_amnt']))
			{
		        ...
		        $rs3=mysql_query("insert into `ledger` set `date`='".datefordb($_POST['date'])."',`name`='Tds Deducted',`ledger_master_id`='".$row_ledger_master_tds['id']."',`credit`='".$_POST['tds_amnt']."',`debit`='0',`invoice_id`='".$invoice_list."',`transaction_type`='receipt',`transaction_id`='".$_POST['receipt_id']."',`narration`='".$_POST['narration']."',`current_date`='".date("Y-m-d")."'");				
			}
